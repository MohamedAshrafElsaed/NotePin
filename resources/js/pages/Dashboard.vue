...
<template>
    ...
    <AppLayout :show-fab="true">
        ...
        <div class="flex-1 px-4 sm:px-6 py-6">
            <!-- Page title -->
            <div class="mb-6">
                <h1 class="text-xl font-semibold text-[#0F172A]">{{ t('notes.myNotes') }}</h1>
                <p class="text-sm text-[#64748B]">{{ recordings.length }} {{ t('notes.recordings') }}</p>
            </div>

            <!-- Search -->
            <div v-if="recordings.length > 3" class="relative mb-6">
            ...
            <!-- Notes list -->
            <div v-if="filteredRecordings.length" class="space-y-3">
                <Link
                ...
                >
                    <div class="flex items-start gap-3">
                        ...
                        <div class="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0"
                             :class="recording.status === 'ready' ? 'bg-[#F0FDF4]' : 'bg-[#EEF2FF]'">
                            <svg v-if="recording.status === 'ready'" class="w-5 h-5 text-[#22C55E]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                            </svg>
                            <svg v-else-if="recording.status === 'processing'" class="w-5 h-5 text-[#F59E0B] animate-spin" fill="none" viewBox="0 0 24 24">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-start justify-between gap-2 mb-1">
                                ...
                                <span
                                    ...
                                    :class="statusConfig[recording.status as keyof typeof statusConfig]?.class || 'bg-gray-100 text-gray-700'"
                                >
                                    {{ statusConfig[recording.status as keyof typeof statusConfig]?.label || recording.status }}
                                </span>
                            </div>
                            <p v-if="recording.ai_summary" class="text-sm text-[#64748B] line-clamp-2 mb-2">
                            ...
                            <div class="flex items-center gap-3 text-xs text-[#64748B]">
                                ...
                                <span v-if="recording.ai_action_items?.length" class="flex items-center gap-1">
                                    ...
                                    {{ recording.ai_action_items.length }}
                                </span>
